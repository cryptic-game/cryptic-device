...
def check_elements_exist(elements: dict) -> Tuple[bool, dict]:
    ...
    for disk in elements["disk"]:
        if disk not in hardware["diskStorage"].keys():
            return False, {"error": "element_disk_not_found"}
    for ram in elements["ram"]:
        if ram not in hardware["ram"].keys():
...
def check_compatible(elements: dict) -> Tuple[bool, dict]:
    exists, message = check_elements_exist(elements)
    if not exists:
