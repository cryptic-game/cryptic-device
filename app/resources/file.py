...
FileGetAllRequestSchema = api.model("Get All File Request", {
    "device": fields.String(example="uuid",
                            description="uuid of device")
})
...
class FileModificationApi(Resource):
    ...
    def get(self, session, uuid):
        ...
        if file is None:  # TODO limit access
            abort(404, "invalid file uuid")

        return file.serialize

    @file_api.doc("Update a file")
    @file_api.marshal_with(FileUpdateRequestSchema)
...
class FileAPI(Resource):
    ...
    def get(self, session):
        ...
        return {
            "files": FileModel.query.filter_by(device=device).all()
        }
    ...
    def put(self, session):
        ...
        file: Optional[FileModel] = FileModel.create(device, filename, content)
