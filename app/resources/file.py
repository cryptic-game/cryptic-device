...
def get_all(data: dict, user: str) -> dict:
    ...
    return {
        'files': [f.serialize for f in m.session.query(File).filter_by(device=device.uuid).all()]
    }
...
def info(data: dict, user: str) -> dict:
    ...
    file: Optional[File] = m.session.query(File).filter_by(uuid=data['file_uuid']).first()
...
def update(data: dict, user: str) -> dict:
    ...
    file_count: int = len(m.session.query(File).filter_by(device=device.uuid).filter_by(filename=data["filename"]).all())
...
def create(data: dict, user: str) -> dict:
    ...
    file_count: int = len(m.session.query(File).filter_by(device=device.uuid).filter_by(filename=filename).all())
